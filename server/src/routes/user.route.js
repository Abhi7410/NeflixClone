...
router.post("/signup",
  body("username")
    ...
    .custom(async value => {
      ...
      if(user) return Error("Username is already in use");
    }),
  body("password")
    .exists().withMessage("password is required")
    .isLength({ min: 10 }).withMessage("password minimum 10 characters"),
  body("confirmPassword")
    .exists().withMessage("confirmPassword is required")
    .isLength({ min: 10 }).withMessage("confirmPassword minimum 10 characters")
    .custom((value, { req }) => {
